...
df = read_csv("20migration.csv")

regions <- c("Africa", "Asia", "Europe", "Latin America", "Northern America", "Oceania")

origins <- c("AFRICA", "ASIA", "EUROPE", "LATIN AMERICA AND THE CARIBBEAN", "NORTHERN AMERICA", 
             "OCEANIA")

origin_pos <- c(1,2, 24, 30, 36, 41, 45, 46, 51)
index <- c(903, 935, 908, 904, 905, 909)
dest_pos <- c(22, 28,34, 39, 43, 44)

names(df)[1] <- 'Destination'
df %>% select(1, origins) %>%
  filter(Destination %in% origins) -> test
  
dm <- data.matrix(test[, 2:7])


dm[upper.tri(dm)] <- dm[lower.tri(dm)] + dm[upper.tri(dm)]
dm[lower.tri(dm)] <- dm[upper.tri(dm)]


test2 <- data.frame(dm)
test2$destination <- regions
test2$id <- index

test3 <- test2 %>%
  select(7,8, 1:6)
names(test3) <- c('destination', 'index', regions)
...
write_csv(test3, "region20_upper_matrix.csv")
